# PFF Draft-Binding Mutation Qualification Profile v0.1

Status: ACCEPTED CONFORMANCE PROFILE

This document freezes only the executable qualification boundary for the
already accepted `pff-draft-binding/0.1` implementation. It does not change a
binder value, gate, diagnostic, ordering rule, fixture outcome, or public API.
...
profile and manifest must be the exact tracked accepted blobs in that tree.
...
<commit>`. The immediate
subject child must satisfy section 13's accepted-authority prospective-tree
rule and exact two-path diff; preflight reads its two accepted blobs with exact
`<GIT> show <accepted-authority-commit>:<repository-relative-authority-path>`
commands in profile-then-manifest order and hashes the complete stdout bytes.
Stderr is empty and exit zero; blob bytes have no Git-added framing. There must
be at least one later commit. The first
later commit has exactly the section-10 implementation allowlist as its path
set; every further repair commit has a nonempty subset that includes exact
`qualification/PFF_DRAFT_BINDING_MUTATION_TOOL_REVIEW_V0.1.json`, and exact `<GIT> diff
--name-only --no-renames --no-ext-diff -z
<accepted-authority-commit>..HEAD` has no path outside that
allowlist. Every inspected commit/tree/blob exists, every command exits zero
...
Successful writes use an adjacent exclusive temporary file, `fchmod` to exact
`0644`, file `fsync`, atomic replace, and parent-directory `fsync`; newly
...
| `authority_identity` | The tracked accepted `<PROFILE>` and `<MANIFEST>` blobs, exact subject→authority→tool first-parent topology and per-commit path rules, observed implementation `HEAD`/tree, then exact script, leaf-adapter, tool-test, and tool-review path/mode/length/hash records. | `AUTHORITY_MISMATCH`; first of `<PROFILE>`, `<MANIFEST>`, `implementation:commit`, `implementation:git_tree`, or the four repository-relative artifact paths in that order |
...
The accepted execution runtime for v0.1 is exact CPython `3.12.13` on
...
in section 9, and no interruption creates a final evidence bundle. v0.1 forbids resume: that
...
atomically creates the ACTIVE envelope defined in section 9, and only then
...
`pff-draft-binding-mutation-evidence/0.1`. Its top-level object has exactly:
...
`docs/PFF_DRAFT_BINDING_MUTATION_QUALIFICATION_PROFILE_V0.1.md` and
...
The separate canonical execution envelope has schema
`pff-draft-binding-mutation-execution-envelope/0.1` and exactly `schema`,
`status`, `campaign_id`, `subject`, `criteria`, `tool_subject`, `preflight_sha256`,
`plan_sha256`, `path_map`, `commands`, `started_at`, and `finished_at`. The five
identity/hash members equal the journal headers. Status is `ACTIVE` or
`COMPLETE`; `finished_at` is null exactly for ACTIVE and is a timestamp exactly
for COMPLETE. `subject` is the manifest subject, `criteria` has exact members
`qualification_profile_sha256` and `manifest_sha256`, and `tool_subject` has
exact members `commit` and `git_tree`. Immediately before the first
runner-direct control spawn, `run` samples
`clock_gettime_ns(CLOCK_REALTIME)`, floors to the containing microsecond, and
formats that UTC instant as `started_at`. Immediately after the COMPLETE
journal's file and parent-directory fsync and before serializing the COMPLETE
envelope, it samples, floors, and formats `finished_at` identically. If the
second value is earlier than `started_at`, terminalization is
`INTERNAL_FAILURE`; otherwise `finished_at >= started_at`. No other instant or
rounding rule is valid. The runner buffers headers, start time, path map, and successful
control observations in private invocation storage while controls run. Only
after every control passes and the empty ACTIVE journal has been durably
created, and before the first mutant-start journal event, it atomically creates
an ACTIVE envelope containing those buffered commands.
It atomically rewrites the envelope after every subsequent complete
direct-child observation, and that rewrite completes before any corresponding
component seal, synthetic construction seal, or mutant seal can be persisted
in the journal and before its classification is retained for eventual
evidence. A primitive failure before a complete command observation or while
making its envelope row durable preserves the prior valid envelope prefix and leaves the already
persisted journal dispatch/start unsealed. A control failure leaves the declared envelope absent; an
interruption after attempts begin leaves a schema-valid ACTIVE envelope bound
to one subject, criteria/tool pair, and plan.
`run` has at most one runner-direct child live at a time. `path_map` uses the exact token order displayed in section 7 and every member has exactly `logical`,
...
completed run. An ACTIVE envelope is the last durable prefix of complete
observations and may omit only a currently in-flight child, a child whose
spawn/exec/pipe/clock/process primitive prevented a complete observation, or a
just-completed child whose envelope rewrite/replace/fsync did not become durable;
only that single current or just-completed child may be absent. A durable row
is never retracted or changed, and an ACTIVE envelope never contains a partial
command object. Timestamps use exact UTC form `YYYY-MM-DDTHH:MM:SS.ffffffZ`.
`execution_envelope_sha256` binds the complete final bytes. No credentials or
inherited environment enter it.

For a COMPLETE envelope and final bundle, every canonical runtime probe, suite, wheel, install, import, construction
...
An interruption after the empty ACTIVE journal is created but before envelope
creation leaves that journal alone with zero events and zero consumed attempts.
An interruption after envelope creation but before the M01 start leaves both
ACTIVE artifacts, the journal still has zero events, and zero attempts were
consumed. Neither state is a campaign result or permits resume; both abandon
the campaign ID and write no bundle.

After the last classified component and the exact post-attempt identity pass,
the runner atomically persists the final mutant seal and thereby the COMPLETE
journal, then marks the envelope COMPLETE
with `finished_at`, then writes the bundle atomically. Interruption before the
last seal after the M01 start leaves ACTIVE journal/envelope; between the last seal and envelope
finalization it leaves a COMPLETE journal plus ACTIVE envelope; between
envelope finalization and bundle publication it leaves COMPLETE journal and
envelope but no bundle. None is silently called a final qualification bundle.
The bundle does not contain its own digest. Its external SHA-256 is computed
...
The authority-only candidate and acceptance tranche changes exactly:

```text
docs/PFF_DRAFT_BINDING_MUTATION_QUALIFICATION_PROFILE_V0.1.md
docs/PFF_DRAFT_BINDING_MUTATION_MANIFEST_V0.1.json
```
...
qualification/PFF_DRAFT_BINDING_MUTATION_TOOL_REVIEW_V0.1.json
README.md
```
...
`src/poietics`, any existing test, either accepted authority file, or package
...
be classified as a kill. It also proves executable reachability and closed
dispatch for all 191 leaves and all 205 component occurrences; pre-acceptance
criteria review does not claim that post-acceptance code already exists.

The tool-review file is canonical JSON schema
`pff-draft-binding-mutation-tool-review/0.1` with exactly `schema`, `status`,
...
is `pff-draft-binding-mutation-post-run-review/0.1` and has exactly `schema`,
...
`qualification/PFF_DRAFT_BINDING_MUTATION_JOURNAL_V0.1.json`,
`qualification/PFF_DRAFT_BINDING_MUTATION_EVIDENCE_V0.1.json`, and
`qualification/PFF_DRAFT_BINDING_MUTATION_EXECUTION_ENVELOPE_V0.1.json`; the
...
```text
qualification/PFF_DRAFT_BINDING_MUTATION_EVIDENCE_V0.1.json
qualification/PFF_DRAFT_BINDING_MUTATION_EXECUTION_ENVELOPE_V0.1.json
qualification/PFF_DRAFT_BINDING_MUTATION_JOURNAL_V0.1.json
qualification/PFF_DRAFT_BINDING_MUTATION_POST_RUN_REVIEW_V0.1.json
README.md
```
...
journal/envelope instead of a complete bundle. Primitive failure includes
spawn/exec, pipe, clock, process-group, signal, drain/wait, file write,
`fsync`, or atomic-replace failure that the termination/component schemas
cannot honestly represent. The runner preserves the last durable journal and
envelope states, whether ACTIVE or COMPLETE, emits the canonical
...
Before acceptance, three independent read-only reviews report:

```text
architecture: qualification boundary and allowlists are clean
determinacy: manifest, leaf, classification, serialization, and replay rules are complete
criteria: every M recipe faithfully represents its forbidden transformation and every expected component vector is semantically reachable from accepted F/C text
```

The reviewed candidate consists of the exact candidate profile bytes and exact
manifest bytes. It is published as one ordinary non-force authority commit
whose direct parent is exact subject commit
`15e61728eec90c45ca644f804e14f4d54dc31e38`. Before commit, an independent
review derives the prospective accepted-profile bytes by the exact
transformation below. The prospective tree is exactly the subject tree plus
those accepted-profile bytes at Git mode `100644` and the reviewed manifest
bytes unchanged at Git mode `100644`; its changed-path set against the parent
is exactly the two section-10 authority paths. The committed tree equals that
prospective tree. After an ordinary non-force push, a re-fetch authenticates
the same commit, direct parent, tree, and both authority blob bytes.
...
```text
profile: pff-draft-binding-mutation-qualification/0.1
status: accepted
...
including their existing final LF. The manifest is not transformed during
acceptance. The record binds the candidate and manifest hashes, never its own
accepted-profile hash or commit, so it is not self-referential. No other profile
or manifest byte may change. Implementation and
controlled mutation execution begin only after the accepted pair is published.
...
reviewed_candidate_sha256: sha256:1700a18108f1de0e064bb071602a9102fde56d414124cdf37c607537c018ce10
recipe_manifest_sha256: sha256:2e967a8a37e90ea63928239040d852225620a42d391951f75ee02f20dea325e2
review_result: architecture=clean; determinacy=clean; criteria=clean
```
